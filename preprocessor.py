...
def error(e):
    print(f'\033[91m\033[1m{e}\033[0m')
        ...
        for dir in dirs:
            for _, _, files in os.walk(directory + '/' + dir):
                ...
                for idx, file in enumerate(files):
                    ...
                    all_data_bandwidth[image_index, :,
                                       0:spec_bw.shape[1]] = spec_bw
                    all_data_stft[image_index, :,
                                  0:chroma_stft.shape[1]] = chroma_stft
                    all_data_cens[image_index, :,
                                  0:chroma_cens.shape[1]] = chroma_cens
                    all_data_cq[image_index, :,
                                0:chroma_cq.shape[1]] = chroma_cq
                    all_data_contrast[image_index, :,
                                      0:contrast.shape[1]] = contrast
...
# function to standarize the data x=(x-mean)/std a.k.a Normalization
def standarize(x):
    mean = x.mean(axis=0)
    std = x.std(axis=0)
    diff = x - mean
...
def min_max_normalize(x):
    ...
    range = max - min
    np_minmax = np.divide(
        diff, range, out=np.zeros_like(diff), where=range != 0)
...
# Splitting data - function to split data into training and testing
# (default is 90% for training and 10% for testing)
def split_data(dataset, labels, num_classes=10, test_images_for_class=10):
    test_counter = np.zeros(num_classes)
    if dataset.ndim == 3:
        test_set = np.zeros((num_classes * test_images_for_class,
                            dataset.shape[1], dataset.shape[2]), dtype=float)
    elif dataset.ndim == 2:
        test_set = np.zeros(
            (num_classes * test_images_for_class, dataset.shape[1]),
            dtype=float)
...
if __name__ == '__main__':
    ...
    train_set_std, train_labels_std, test_set_std, test_labels_std = \
        split_data(all_data_standard, all_labels)
    eraseLastLine()
    print("Splitting ✅")
